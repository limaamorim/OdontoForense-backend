...
exports.criarVitima = async (req, res) => {
  try {
    ...
    const novaVitima = new Vitima({ ...req.body, casos: [casoId] });
...
exports.listarVitimasPorCaso = async (req, res) => {
  try {
    const { casoId } = req.params;
    const vitimas = await Vitima.find({ casos: casoId });
    res.json(vitimas);
  } catch (err) {
...
exports.atualizarVitima = async (req, res) => {
  try {
    ...
    if (!vitimaAtualizada) return res.status(404).json({ msg: 'Vítima não encontrada' });
